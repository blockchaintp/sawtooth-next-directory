...
def address_is(address):
    ...
    if _contains(addr1, SysAdminNamespace.SYS_ADMIN_START,
        ...
        raise ValueError("Unable to determine state location of address %s",
                         address)
...
def _sysadmin_address_is(address):
    ...
    elif _contains(num, SysAdminRelationshipNS.SYSADMIN_ADMINS_START,
...
def _role_address_is(address):
    ...
    if _contains(num, RoleRelationshipNamespace.ROLE_ATTRIBUTES_START,
...
def _compress(object_id, start, limit):
    return "%.2X".lower() % (int(
        sha512(object_id.encode()).hexdigest(),
        base=16) % limit + start)
...
def make_role_members_address(role_id, user_id):
    return _make_role_address(role_id) + \
           _compress(
               user_id,
               RoleRelationshipNamespace.ROLE_MEMBER_START,
               RoleRelationshipNamespace.ROLE_MEMBER_STOP -
...
def make_task_owners_address(task_id, user_id):
    return _make_task_address(task_id) + \
           _compress(
               user_id,
               TaskRelationshipNamespace.TASK_OWNER_START,
               TaskRelationshipNamespace.TASK_OWNER_STOP -
